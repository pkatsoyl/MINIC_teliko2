...
%start compile_unit
%token <node> IDENTIFIER NUMBER INT FLOAT
%token RETURN BREAK CONTINUE IF ELSE WHILE DO FOR
%nonassoc IFPREC
%left '='
%left OR
%left AND
...
%left MULT DIV
%nonassoc NOT ELSE
%type <node> compile_unit function_definition fargs statement if_statement while_statement dowhile_statement for_statement compound_statement statement_list expression args
%%
...
function_definition : type_sp IDENTIFIER '(' fargs ')' compound_statement { $$ = new CFunctionDefinition($2,$4,$6);
																			g_symbolTable.GetSymbol(((CIDENTIFIER *)$2)->m_text)->syntaxNode =$$;
																		   }
				|	  type_sp IDENTIFIER '('  ')' compound_statement  { $$ = new CFunctionDefinition($2,$5);
																		g_symbolTable.GetSymbol(((CIDENTIFIER *)$2)->m_text)->syntaxNode =$$;
																	   }
				;

fargs :   type_sp IDENTIFIER		{ $$ = new CFormalArgs($2); }
		| fargs ',' fargs  { $$ = new CFormalArgs($1,$3); }
		;
...
dowhile_statement : DO statement WHILE '(' expression ')'   { $$ = new CDoWhileStatement($2,$5) ;}
			;

for_statement : FOR '(' expression ';' expression ';' expression ')' statement  { $$ = new CForStatement($3,$5,$7,$9) ;}
			;
