...
CStatement::CStatement(NodeType tp, int children, CSTNode* arg1, CSTNode* arg2, CSTNode* arg3, CSTNode* arg4) : CSTNode(tp, children, arg1, arg2, arg3,arg4) {}
...
CDoWhileStatement::CDoWhileStatement(CSTNode* body, CSTNode* cond) :CStatement(NT_DOWHILESTATEMENT, 2, body, cond) {}
CForStatement::CForStatement(CSTNode* cond1, CSTNode* cond2, CSTNode* cond3, CSTNode* body) :CStatement(NT_FORSTATEMENT, 4, cond1, cond2, cond3, body) {}
...
CContinueStatement::CContinueStatement() :CStatement(NT_CONTINUESTATEMENT, 0) {}
...
void CDoWhileStatement::PrintSyntaxTree(ofstream* dotfile, CSTNode* parent) {
	(*dotfile) << "\"" << parent->GetGraphVizLabel() << "\"->\"" << GetGraphVizLabel() << "\";\n";
	CSTNode::PrintSyntaxTree(dotfile, this);
}
void CForStatement::PrintSyntaxTree(ofstream* dotfile, CSTNode* parent) {
	(*dotfile) << "\"" << parent->GetGraphVizLabel() << "\"->\"" << GetGraphVizLabel() << "\";\n";
	CSTNode::PrintSyntaxTree(dotfile, this);
}
...
void CContinueStatement::PrintSyntaxTree(ofstream* dotfile, CSTNode* parent) {
	(*dotfile) << "\"" << parent->GetGraphVizLabel() << "\"->\"" << GetGraphVizLabel() << "\";\n";
	CSTNode::PrintSyntaxTree(dotfile, this);
}
...
double CDoWhileStatement::Evaluate(CSTNode* parent) {

	do	{
		GetChild(0)->Evaluate(this);
		if (m_breakVisit) {
			m_breakVisit = false;
			break;
		}
	...
	} while (GetChild(1)->Evaluate(this));
	return 0;
}
double CForStatement::Evaluate(CSTNode* parent) {
	
	for (GetChild(0)->Evaluate(this); GetChild(1)->Evaluate(this); GetChild(2)->Evaluate(this)) {
			
		GetChild(3)->Evaluate(this);
...
double CContinueStatement::Evaluate(CSTNode* parent) {
